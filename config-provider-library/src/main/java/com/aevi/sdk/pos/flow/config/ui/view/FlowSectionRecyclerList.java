...
public class FlowSectionRecyclerList extends LinearLayout implements FlowAppViewAdapter.OnFlowSectionListener {
    ...
    @BindView(R2.id.section_flow_app_type)
    TextView flowAppType;

    @BindView(R2.id.flow_app_list)
    ...
    public void setFlowApps(List<AppInfoModel> flowApps, int[] colorArray) {
        if (flowApps != null) {
            if (adapter == null) {
                adapter = new FlowAppViewAdapter(flowApps, this, colorArray);
